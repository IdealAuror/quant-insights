...
def main():
    ...
    # TOM window: T-2 to T+5
    tom = rdf[rdf["rel_day"].between(-2, 5)]
    ...
    print("\n" + "="*70)
    print("  TOM WINDOW [T-2 ~ T+5] DAILY BREAKDOWN")
    print("="*70)
    print(f"  {'Day':>5}  {'Loser':>10}  {'Winner':>10}  {'WML':>10}  {'L_t':>7}  {'L_p':>8}  {'H_t':>7}  {'H_p':>8}  Sig(L)")
    print("-"*70)

    stats_list = []
    for d in range(-2, 6):
        sub = tom[tom["rel_day"] == d]
        if len(sub) == 0:
            continue
        ...
        stats_list.append({"d": d, "label": label, "lo": lo.mean()*1e4, "hi": hi.mean()*1e4, "wml": wml_day.mean()*1e4,
                           "t_lo": t_lo, "p_lo": p_lo, "t_hi": t_hi, "p_hi": p_hi, "sig_hi": sig_hi,
                           "t_wml": t_wml, "p_wml": p_wml})
        print(f"  {label:>5}  {lo.mean()*1e4:>+9.2f}  {hi.mean()*1e4:>+9.2f}  {wml_day.mean()*1e4:>+9.2f}  "
              f"{t_lo:>+6.2f}  {p_lo:>7.4f}  {t_hi:>+6.2f}  {p_hi:>7.4f}  {sig_lo}")
    ...
    cum = sum(lo_vals)
    t1_val = [s["lo"] for s in stats_list if s["d"] == 1][0] if any(s["d"] == 1 for s in stats_list) else 0
    print(f"\nTOM cumulative loser alpha: {cum:.1f} bps")
    print(f"T+1 loser rebound: {t1_val:.1f} bps")
    if cum != 0:
        print(f"T+1 share of total TOM rebound: {t1_val/cum*100:.1f}%")
    ...
    for b, v in zip(bars, vals):
        ax.text(b.get_x()+b.get_width()/2, b.get_height()+(1 if v>=0 else -3),
    ...
    ax.set_ylabel("bps"); ax.set_title("Loser (D1) daily return in TOM window"); ax.axhline(0, color="k", lw=0.8); ax.legend()
    ...
    ax.set_ylabel("bps"); ax.set_title("Winner (D10) daily return in TOM window"); ax.axhline(0, color="k", lw=0.8); ax.legend()
    ...
    ax.set_ylabel("bps"); ax.set_title("WML daily in TOM window"); ax.axhline(0, color="k", lw=0.8); ax.legend()

    plt.tight_layout()
    out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tom_daily.png")
